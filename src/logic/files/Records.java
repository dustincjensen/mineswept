...
public class Records {
				...
				if(line.matches("\\[.*\\]")) {
					if(line.equals("[Beginner]")) {
						for(int i=0; i<(RECORD_LIMIT*3); i++) {
							if((line = br.readLine()).equals("[Intermediate]")) break;
							else beginner.add(line);
						}
					}
					if(line.equals("[Intermediate]")) {
						for(int i=0; i<(RECORD_LIMIT*3); i++) {
							if((line = br.readLine()).equals("[Advanced]")) break;
	...
	public static boolean addNewRecord(String diff, String name, String time) {
		...
		String date = ""+cal.get(cal.DAY_OF_MONTH)+
		              "/"+(cal.get(cal.MONTH)+1)+
		              "/"+cal.get(cal.YEAR);
