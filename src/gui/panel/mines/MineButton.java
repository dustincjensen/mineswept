...
public class MineButton extends JLabel implements MouseListener {
	...
	/*
		inside squares fixes the cursor leaving the buttons and still clicking the last answered
		but I need a listener on the panel to know when it leaves to change the face and to know when
		the mouse was released from a press that starts outside of the buttons or else MineButtons
		don't get the release event.
	*/
	...
	*/
	...
	public void decorate() {
		...
		if(t.uncovered()) {
			...
			if (t.blewUp())	setBackground(Color.RED);
		...
		// Still covered
			...
			if (!GameFeatures.isGameOver() && t.isHint()) {
				if (t.isSpecialProtected())	setIcon(flagHint);
				else						setIcon(hint);
				setText("");
				t.setHint(false);
			}
	...
	private static int parseIndividualColor(String c) {
		...
		else if(c.indexOf("g") != -1) return 2;
		else if(c.indexOf("b") != -1) return 3;
		else {
			useDefaultBackgroundColor();
